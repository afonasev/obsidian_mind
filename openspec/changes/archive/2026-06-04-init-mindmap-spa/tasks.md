...
- [x] 16.15 Новый узел добавляется последним среди соседей: домен `appendChildY`, используется в `childHintPosition` (стор) и `createSiblingOf` (Canvas)

## 15. Финальная проверка

- [x] 15.1 Запустить `make check` локально — должно быть зелёным, coverage 100%
- [x] 15.2 Запустить `bun run tauri dev` — открыть окно, создать пару корневых узлов, создать у одного из них ребёнка, отредактировать текст, перетащить, удалить, перезапустить — убедиться, что граф восстановился
